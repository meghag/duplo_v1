...
namespace sort_duplos {
	...
	int DataProcessor::find_state(pcl::PointCloud<PointXYZRGB>::Ptr cloud, int region_id)
	{
		...
		vector<sensor_msgs::PointCloud2> clusters = cluster(cloud,region_id);
		vector<int> count;
	...
	sensor_msgs::PointCloud2 DataProcessor::find_central_duplo(pcl::PointCloud<PointXYZRGB>::Ptr pcd_ptr)
	{
	...
	geometry_msgs::Point DataProcessor::find_centroid(sensor_msgs::PointCloud2 cloud2)
	{
		...
		centroid.x = sumx/cloud->points.size();
		centroid.y = sumy/cloud->points.size();
		centroid.z = sumz/cloud->points.size();
	...
	void DataProcessor::assign_manipulation_primitive()
	{
		...
		for (size_t i = 0; i < state_.size(); i++) {
			if (state_[i].second == 1) {
				...
				cluttered_region_ids.push_back(i);
				//Find central duplo
				sensor_msgs::PointCloud2 central2 = find_central_duplo(state_[i].first);
				geometry_msgs::Point centroid = find_centroid(central2);
				
				//Find possible directions of spread
				vector<geometry_msgs::Point> extent = find_extents(*(state_[i].first));
				region_extents_map[i] = extent;
				spread_directions_map[i].resize(2);
				spread_directions_map[i][0] = 0;
				spread_directions_map[i][1] = 0;
				if (abs(centroid.x - extent[0].x) < abs(centroid.x - extent[1].x)) {
					if (extent[0].x - table_extent_[0].x > 0.1)
						//Spread in -x
						spread_directions_map[i][0] = -1;
				} else {
					if (-extent[1].x + table_extent_[1].x > 0.1)
						//Spread in +x
						spread_directions_map[i][0] = 1;
				}

				if (abs(centroid.y - extent[2].y) < abs(centroid.y - extent[3].y)) {
					if (extent[2].y - table_extent_[2].y > 0.1)
						//Spread in -y
						spread_directions_map[i][1] = -1;
				} else {
					if (-extent[3].y + table_extent_[3].y > 0.1)
						//Spread in +y
						spread_directions_map[i][1] = 1;
				}
			}					
		...
		for (size_t i = 0; i < state_.size(); i++) {
			if (state_[i].second == 2) {
				//Find neighboring cluttered and piled regions
				neighbors[i].clear();
				for (size_t j = i+1; j < state_.size(); j++) {
					if (state_[j].second < 2)
						continue;
					if (spread_directions_map[i][0] == -1 && spread_directions_map[i][1] == -1) {
						double diffx = region_extents_map[i][0].x - region_extents_map[j][1].x;
						double diffy = region_extents_map[i][2].y - region_extents_map[j][3].y;
						if (diffx > 0 && diffx < 0.1)
							spreadx = 0;
						
				}

				//If spread won't collide with neighboring regions that haven't been spread, spread this region

				//If spread, mark this region has been spread...use this info for deciding whether to spread neighboring regions
				has_region_been_manipulated[i] = true;
