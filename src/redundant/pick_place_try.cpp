...
public:
    PickPlace(ros::NodeHandle & n): n_(n), collision_object_current_id_(0) {
        ...
        */
        ...
        graspDuploServer_ = n_.advertiseService("grasp_duplo", &PickPlace::graspDuploCallback, this);
...
private:
    ...
    bool graspDuploCallback (duplo_v1::Grasp_Duplo::Request &req,
    		duplo_v1::Grasp_Duplo::Response &res)
    {
    	...
    	clusters_ = req.pick_cluster;
    	ROS_INFO("Got %zu clusters", clusters_.size());
    	//PickupObject();
    ...
    bool pickup(int cluster_id) {
        ...
        //Sanity check
        sensor_msgs::PointCloud2 tgo2;
        sensor_msgs::convertPointCloudToPointCloud2(target_graspable_object_.cluster, tgo2);
        tgo2.header.frame_id = "base_link";
        target_object_pub_.publish(tgo2);
        ...
        object.dimensions[0] = bboxes_[cluster_id].dimensions.x + 0.02; // need large padding in x direction if push is in x direction bcoz after push the hand will be in collision
        object.dimensions[1] = bboxes_[cluster_id].dimensions.y + 0.02;
        object.dimensions[2] = bboxes_[cluster_id].dimensions.z + 0.1;
        co.shapes.push_back(object);

        co.poses.push_back(bboxes_[cluster_id].pose_stamped.pose);
        //pickup_goal.additional_collision_operations.collision_operations._objects.push_back(co);
        //arm_navigation_msgs::CollisionObject reset_object;
        ...
        planning_scene_srv.request.operations.collision_operations.push_back(cop2);
