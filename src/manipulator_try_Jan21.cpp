...
namespace sort_duplos
{
	...
	bool Manipulator::pick_n_place()
	{
		...
		temp.z += 0.2;
		if (move_arm_pp(temp, 1, 0) == 0) {
			temp.z -= 0.18;
			if (move_arm_pp(temp, 1, 0) == 0) {
				gripper_.close();
				temp.z += 0.2;
				move_arm_pp(temp, 1, 0);
				move_arm_spread(findPlaceLocation(), 1, 1, 2);
				gripper_.open();
			} else {
				move_arm_spread(findPlaceLocation(), 1, 1, 2);
				return false;
...
//		breakpoint();
	...
	bool Manipulator::spread()
	{
		...
		for (int plan_id = 1; plan_id <= 1; plan_id++) {
			if (move_arm_spread(spread_path_[0], 1, 0, plan_id) != -1)
			{
				move_count++;
	...
	int Manipulator::move_arm_pp(geometry_msgs::Point go_to, int cluster_id, int cluster_op)
	{
		actionlib::SimpleActionClient<arm_navigation_msgs::MoveArmAction> move_arm("move_right_arm",true);
		move_arm.waitForServer();
		ROS_INFO("DUPLO: Connected to server");
		...
		desired_pose.link_name = "r_wrist_roll_link";
		desired_pose.pose.position = go_to;
		desired_pose.pose.position.x -= 0.015;
