 ...
 *  
 *
 *  Created by Megha Gupta on 04/15/13.
 *  Copyright 2013 USC. All rights reserved.
 *
 */

#ifndef PROCESS_PCD_ALT_H
#define PROCESS_PCD_ALT_H
...
namespace sort_duplos {
	...
	private:
		...
		sensor_msgs::PointCloud2 find_central_duplo(pcl::PointCloud<PointXYZRGB>::Ptr pcd_ptr);
		geometry_msgs::Point find_centroid(sensor_msgs::PointCloud2 cloud2);
		...
		map<int, vector<geometry_msgs::Point> > region_extents_map;		//Map of region id to that region's extents
		map<int, bool> has_region_been_manipulated;		//Map of region id to whether it has been manipulated
		map<int, vector<int> > spread_directions_map;		// Map of region id to its spread directions (+1, -1 in each x and y direction)
		map<int, vector<int> > neighbors;	
		vector<int> cluttered_region_ids;
	};
}
